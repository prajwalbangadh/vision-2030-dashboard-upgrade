(function(){
  ...
  function sourceChip(){
    return `<div class="source-chip"><strong>Source workbook batch</strong>${esc(D._source.outputBatch)}<br>${fmtInt(D.quality.detailRows)} detail rows · ${fmtInt(D.quality.uniqueScopes)} scopes</div>`;
  }
  ...
  // ---------- Briefing ----------
  const briefingState={level:safeStorageGet('v30-brief-level','System'),division:safeStorageGet('v30-brief-division','Central Florida Division')};
  function directBriefingDivisions(){
    const preferred=['Central Florida Division','East Florida Division','West Florida Division','Primary Health Division','Multi-State Division','Multi-State Division 1','Multi-State Division 2','Corporate Services'];
  ...
  // ---------- Data Explorer ----------
  const explorerState={area:safeStorageGet('v30-ex-area','Vision 2030'),view:safeStorageGet('v30-ex-view','scorecard'),divisionBasis:safeStorageGet('v30-ex-division-basis','matched'),metric:'All',division:'All',type:'All',status:'All',date:'All',search:'',sort:'metric',direction:1};
  function explorerDivisionValue(record){return explorerState.divisionBasis==='source'?record.sourceDivision:record.divisionKey}
  ...
  function ensureExplorerState(){
    ...
    const divisions=sortText(typeRows.map(explorerDivisionValue));
    ...
    const divisionRows=typeRows.filter(r=>explorerState.division==='All'||explorerDivisionValue(r)===explorerState.division);
    const dates=sortText(divisionRows.map(r=>r.dataAsOf));
  ...
  function filteredExplorerRows(){
    ...
    return baseExplorerRows().filter(r=>(explorerState.metric==='All'||r.metric===explorerState.metric)&&(explorerState.division==='All'||explorerDivisionValue(r)===explorerState.division)&&(explorerState.type==='All'||r.locationType===explorerState.type)&&(explorerState.status==='All'||r.goalStatus===explorerState.status)&&(explorerState.date==='All'||r.dataAsOf===explorerState.date)&&(!query||[r.metric,r.location,r.divisionKey,r.matchedDivision,r.sourceDivision,r.reportGeography,r.mappingStatus,r.locationType,r.valueDisplay,r.goalDisplay].some(v=>String(v).toLowerCase().includes(query)))).sort((a,b)=>{
  ...
  function explorerControls(){
    ...
    const divisionBasisLabel=explorerState.divisionBasis==='source'?'Source division':'Matched division';
    const divisionOptions=(divisions.length>1?option('All',optionLabel(`All ${divisionBasisLabel.toLowerCase()}s`,typeRows.length),explorerState.division==='All'):'')+divisions.map(d=>option(d,optionLabel(divisionLabel(d),typeRows.filter(r=>explorerDivisionValue(r)===d).length),d===explorerState.division)).join('');
  ...
  function explorerComposition(rows){
    ...
    else if(explorerState.division==='All'){key=explorerDivisionValue;title=`Available records by ${explorerState.divisionBasis} division`;sub=explorerState.divisionBasis==='source'?`Counts of filtered ${explorerState.type} records using the workbook's unchanged Division field.`:`Counts of filtered ${explorerState.type} records using the separate matching audit. These are available rows, not a completeness rate.`}
  ...
  function bindExplorer(){
    ...
    $('#ex-division-basis').addEventListener('change',e=>{explorerState.divisionBasis=e.target.value;safeStorageSet('v30-ex-division-basis',explorerState.divisionBasis);explorerState.division='All';explorerState.date='All';renderExplorer()});
