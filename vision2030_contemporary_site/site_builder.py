...
DIVISION_LABELS = {
    ...
    "CFD": "CFD", "EFD": "EFD", "WFD": "WFD", "PHD": "PHD",
    "MSD": "MSD", "MSD 1": "MSD 1", "MSD 2": "MSD 2",
...
def latest(root: Path) -> Path:
    ...
    if not files:
        raise FileNotFoundError(f"No business workbook found under {root}")
...
def display_value(raw: Any, source_metric: str, *, summary: bool) -> tuple[str, float | None]:
    text, number = clean(raw) or "N/A", parse_numeric(raw)
    if source_metric in CLINICAL_AGGREGATES:
        if summary and number is not None:
...
def derived_status(
    ...
    if summary and source_metric in CLINICAL_AGGREGATES:
...
def normalize_record(row: dict[str, Any], sheet: str, *, summary: bool, order: int) -> dict[str, Any]:
    ...
    value_text, value_number = display_value(row.get("Value"), source_metric, summary=summary)
    unit = METRIC_UNITS.get(source_metric, "number")
    goal_operator, goal_number = parse_goal(row.get("Goal"), source_metric)
    status, basis, gap = derived_status(source_metric, value_number, goal_operator, goal_number, row.get("Variance"), summary=summary)
...
def build_model(workbook: dict[str, list[dict[str, Any]]], workbook_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "_source": {
            ...
            "outputBatch": workbook_path.parent.name,
...
def write_audit(model: dict[str, Any]) -> dict[str, Any]:
    ...
    rows = [{
        ...
        "Division": r["divisionKey"], "Location Type": r["locationType"], "Location": r["location"],
    ...
    summary = {
        ...
        "generatedMetrics": len(model["metrics"]), "auditPass": model["quality"]["duplicateRows"] == 0,
    }
...
def main() -> None:
    ...
    model = build_model(read_xlsx(workbook_path), workbook_path); validation = write_audit(model)
