...
METRIC_RULES = [
    ...
    },
    {
        ...
        "display_name": "Annual Active Users",
        "metric_id": 8,
        "api_names": [
            "Annual Active Users",
            "Digital Tool Utilization",
            "CNS App Unique Users YTD",
        ],
    ...
    },
    {
        ...
        "display_name": "All-Adult Inpatient Mortality",
        "metric_id": None,
        "api_names": [
            "All-Adult Inpatient Mortality",
            "Premier All-Adult Inpatient Mortality",
        ],
...
# ============================================================
# FINAL DIVISION NORMALIZATION AND AUDIT
# ============================================================

# These mappings use the supplied June 2026 Vision 2030 report.
#
# Rules:
# 1. Location names are preserved for Corporate, Facility, Market,
#    Region, and Campus rows.
# 2. Confirmed location evidence is used only to normalize Division.
# 3. Division-level location names may be abbreviated when confirmed.
# 4. Standalone Multi-State Division becomes MSD, not MSD 1 or MSD 2.
# 5. MSD 1 or MSD 2 is assigned only when a confirmed region or
#    facility provides enough evidence.
# 6. Market values are not used for mapping unless separately confirmed.
# 7. Unsupported values remain unchanged and are written to the audit.
...
MSD1_REGIONS = (
    ...
    "Southeast Region",
    "South East Region",
    "Southeast Region Cost Centers by Reporting Hierarchy",
...
MSD2_REGIONS = (
    ...
    "Southwest Region",
    "South West Region",
    "Southwest Region Cost Centers by Reporting Hierarchy",
...
UNSUPPORTED_CORPORATE_SERVICES_NAMES = {
    "corporate services",
...
def _normalized_text(value):
    """Return consistent case-insensitive text for exact comparisons."""
    return " ".join(clean_text(value).split()).casefold()


def _exact_name_match(value, names):
    """Return a confirmed name when the complete value exactly matches."""
    ...
    for name in names:
        if normalized_value == _normalized_text(name):
            return name

    return ""


def _find_name_in_evidence(evidence_values, names):
    """
    ...
    for value in cleaned_values:
        exact_match = _exact_name_match(value, names)
        if exact_match:
            return exact_match
...
def normalize_division_for_output(node):
    ...
    Unsupported Market and Corporate values remain unchanged.
    """
    original_division = (
        clean_text(node.get("Division"))
        or "N/A"
    )

    location_type = clean_text(
        node.get("Location Type")
    )
    ...
    parent_location = clean_text(
        node.get("Parent Location")
    )

    hierarchy_path = clean_text(
        node.get("Hierarchy Path")
    )
    ...
    # Market names are not confirmed by the supplied report.
    ...
    msd2_region_match = _find_name_in_evidence(
        evidence_values,
        MSD2_REGIONS,
    )
    ...
    if msd1_region_match:
        ...
        # Facility evidence can assign Division while preserving Location.
        cfd_location_match = _find_name_in_evidence(
            evidence_values,
            CFD_LOCATIONS,
        )

        efd_location_match = _find_name_in_evidence(
            evidence_values,
            EFD_LOCATIONS,
        )

        wfd_location_match = _find_name_in_evidence(
            evidence_values,
            WFD_LOCATIONS,
        )

        msd1_location_match = _find_name_in_evidence(
            evidence_values,
            MSD1_LOCATIONS,
        )

        msd2_location_match = _find_name_in_evidence(
            evidence_values,
            MSD2_LOCATIONS,
        )
        ...
        if cfd_location_match:
            ...
            # Apply direct division replacement only to complete individual
            # hierarchy values, never to a partial substring.
            direct_match = ""

            for value in evidence_values:
                normalized_value = _normalized_text(value)

                if normalized_value in DIRECT_DIVISION_MAP:
                    direct_match = normalized_value
                    break

            if direct_match:
                final_division = DIRECT_DIVISION_MAP[direct_match]
                mapping_status = "CONFIRMED_DIRECT_DIVISION"
                matched_evidence = direct_match
                ...
                for value in evidence_values:
                    ...
                    if normalized_value in MULTISTATE_DIVISION_NAMES:
                        multistate_match = normalized_value
                        break

                if multistate_match:
                    # There is insufficient evidence to choose MSD 1 or MSD 2.
                    final_division = "MSD"
                    mapping_status = "CONFIRMED_CONSOLIDATED_MULTISTATE"
                    matched_evidence = multistate_match

                else:
                    corporate_services_match = ""
                    ...
                    for value in evidence_values:
                        ...
                        if (
                            ...
                            corporate_services_match = normalized_value
                            break

                    if corporate_services_match:
                        # Preserve the source value because the supplied
                        # report did not establish a confirmed replacement.
                        final_division = original_division
                        mapping_status = (
                            "UNSUPPORTED_CORPORATE_SERVICES_PRESERVED"
                        )
                        matched_evidence = corporate_services_match

                    elif is_market_row:
                        final_division = original_division
                        mapping_status = "MARKET_PRESERVED_UNCONFIRMED"
                        matched_evidence = location or original_division

                    elif original_division not in ("", "N/A"):
                        final_division = original_division
...
def save_division_mapping_audit():
    """Save every division-normalization decision to a CSV file."""
    audit_file = (
        OUTPUT_FOLDER
        / f"division_mapping_audit_{TIMESTAMP}.csv"
    )
    ...
    with audit_file.open(
        ...
        writer.writeheader()
        writer.writerows(DIVISION_MAPPING_AUDIT)

    print(f"Division mapping audit saved: {audit_file}")
# ============================================================
...
def build_business_table(records):
    ...
    expected_metric_count = len(METRIC_RULES)

    if len(business_records) != expected_metric_count:
        raise ValueError(
            f"Expected exactly {expected_metric_count} business metrics, "
...
def prepend_clinical_level_zero_rows(rows_by_category, business_records):
    clinical_names = {
        "All-Adult Inpatient Mortality",
        "Length of Stay O/E",
        "CMS Star Rating",
    ...
    for record in business_records:
        metric_name = clean_text(record.get("Metric"))
        if metric_name not in clinical_names:
            continue
        level_zero_rows.append({
            ...
            "Variance Direction": record.get("Variance Direction", "→"),
            "Variance": record.get("Variance", "N/A"),
    ...
    existing = [
        row for row in rows_by_category.get("CLINICAL", [])
        if not (row.get("Level") == 0 and row.get("Metric") in clinical_names)
    ]
...
def validate_updated_configuration():
    names = [rule["display_name"] for rule in METRIC_RULES]
    orders = [rule["order"] for rule in METRIC_RULES]
    if len(METRIC_RULES) != 21:
        raise ValueError(f"Expected 21 metric rules, found {len(METRIC_RULES)}.")
    if orders != list(range(1, 22)):
    ...
    for required in (
        ...
        if required not in names:
            raise ValueError(f"Missing required metric rule: {required}")
...
def main():
    validate_updated_configuration()
