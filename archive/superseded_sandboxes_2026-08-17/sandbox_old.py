...
# These page-membership lists come from the supplied June 2026 report.
# A row is renamed only when a direct division name or a recognized
# facility/region is present in its extracted hierarchy evidence.
# Unresolved values are preserved and written to an audit CSV.

CFD_LOCATIONS = (
    "Altamonte Springs", "Apopka", "Celebration", "East Orlando",
    "Kissimmee", "Minneola", "Orlando", "Winter Garden", "Winter Park",
)

EFD_LOCATIONS = (
    "Daytona Beach", "DeLand", "Fish Memorial", "New Smyrna Beach",
    "Ocala", "Palm Coast Parkway", "Palm Coast", "Waterman",
)

WFD_LOCATIONS = (
    "Carrollwood", "Connerton", "Dade City", "Heart of Florida",
    "Lake Placid", "Lake Wales", "North Pinellas", "Port Charlotte",
    "Riverview", "Sebring", "Tampa", "Wauchula", "Wesley Chapel",
    "Zephyrhills",
)
...
MSD1_LOCATIONS = (
    "Avista", "Castle Rock", "Littleton", "Parker", "Porter", "Gordon",
    "Hendersonville", "Manchester", "Murray", "Polk", "Redmond",
)
...
MSD2_LOCATIONS = (
    "Durand", "Lenexa City Center", "Ottawa", "Shawnee Mission",
    "South Overland Park", "UCM AH Bolingbrook", "UCM AH Glen Oaks",
    "UCM AH Hinsdale", "UCM AH La Grange", "Central Texas",
    "Rollins Brook", "TH Huguley", "TH Mansfield",
)
...
def _contains_name(evidence, names):
    evidence_cf = clean_text(evidence).casefold()
    for name in names:
        if clean_text(name).casefold() in evidence_cf:
            return name
    return ""


def normalize_division_for_output(node):
    original_division = clean_text(node.get("Division")) or "N/A"
    location = clean_text(node.get("Current Location"))
    hierarchy_path = clean_text(node.get("Hierarchy Path"))
    parent_location = clean_text(node.get("Parent Location"))
    evidence = " | ".join(
        value for value in (
            location,
            parent_location,
    ...
    direct_value = DIRECT_DIVISION_MAP.get(original_division.casefold())
    if direct_value:
        return direct_value, "CONFIRMED_DIRECT", original_division

    broad_division = original_division.casefold()
    is_florida = broad_division == (
        "florida division cost centers by reporting hierarchy"
    )
    is_multistate = broad_division in {
    ...
    # Direct explicit division evidence can resolve N/A or broad rows.
    for source_name, final_name in (
        ...
        if source_name.casefold() in evidence.casefold():
            return final_name, "CONFIRMED_HIERARCHY", source_name

    if is_florida or original_division == "N/A":
        for final_name, names in (
            ("CFD", CFD_LOCATIONS),
            ("EFD", EFD_LOCATIONS),
            ("WFD", WFD_LOCATIONS),
        ):
            matched = _contains_name(evidence, names)
            if matched:
                return final_name, "CONDITIONAL_LOCATION", matched

    if is_multistate or original_division == "N/A":
        # Region evidence is stronger than facility-name evidence.
        for final_name, names in (
            ("MSD 1", MSD1_REGIONS),
            ("MSD 2", MSD2_REGIONS),
        ):
            matched = _contains_name(evidence, names)
            if matched:
                return final_name, "CONDITIONAL_REGION", matched

        for final_name, names in (
            ("MSD 1", MSD1_LOCATIONS),
            ("MSD 2", MSD2_LOCATIONS),
        ):
            matched = _contains_name(evidence, names)
            if matched:
                return final_name, "CONDITIONAL_LOCATION", matched

    # Heartland and Corporate Services are intentionally not forced.
    # The supplied report does not establish a safe final page mapping.
    return original_division, "REVIEW_UNRESOLVED", ""


def save_division_mapping_audit():
    audit_file = OUTPUT_FOLDER / f"division_mapping_audit_{TIMESTAMP}.csv"
    columns = [
        "Metric", "Level", "Location Type", "Location",
        "Original Division", "Final Division", "Mapping Status",
        "Matched Evidence", "Hierarchy Path",
    ]
    with audit_file.open("w", newline="", encoding="utf-8-sig") as handle:
