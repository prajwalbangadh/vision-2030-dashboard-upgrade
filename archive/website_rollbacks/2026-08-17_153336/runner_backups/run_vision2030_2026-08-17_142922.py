...
def build(template,book,workbook_path):
 ...
 for r in details:
  ...
  if sid not in seen:
   ...
   scopes.append({'id':sid,'label':friendly_scope_label(r.get('Location Type'),r.get('Location'),r.get('Division')),'kind':classify(r.get('Location Type')),'parent':parent,'asp':0,'sourceType':text(r.get('Location Type'))}); seen.add(sid)
  key=(sid,ui)
