...
DIV_LABEL={'CFD':'Central Florida Division','EFD':'East Florida Division','WFD':'West Florida Division','PHD':'Primary Health Division','MSD':'Multi-State Division','MSD 1':'MSD 1','MSD 2':'MSD 2','Corporate Services Division Cost Centers by Reporting Hierarchy':'Corporate Services','Florida Division Cost Centers by Reporting Hierarchy':'Florida Division (Unresolved)','Multistate Division Cost Centers by Reporting Hierarchy':'Multistate Division (Unresolved)'}
...
def scope_id(location_type,location,division):
 lt=text(location_type); loc=text(location)
 if lt.lower()=='corporate' and loc.casefold()=='adventhealth': return 'SYS'
 if lt.lower()=='division':
  for key,label in DIV_LABEL.items():
   if loc.casefold() in {key.casefold(),label.casefold()} or text(division).casefold()==key.casefold(): return key if key in {'CFD','EFD','WFD','PHD','MSD','MSD 1','MSD 2'} else 'DIV::'+slug(key)
...
def build(template,book,workbook_path):
 ...
 scopes=[{'id':'SYS','label':'AdventHealth System','kind':'system','parent':None,'asp':0}]; seen={'SYS'}
 ...
 for div in DIV_ORDER:
  if div in observed_div or div in {'CFD','EFD','WFD','PHD','MSD','MSD 1','MSD 2'}:
   ...
   scopes.append({'id':sid,'label':DIV_LABEL[div],'kind':'division','parent':parent,'asp':0}); seen.add(sid)
 ...
 scopes.append({'id':'UNCLASS','label':'Unclassified / Other','kind':'division','parent':'SYS','asp':0}); seen.add('UNCLASS')
 all_types=sorted({text(r.get('Location Type')) for r in details})
 for lt in all_types:
  sid='UNCLASS::'+slug(lt); scopes.append({'id':sid,'label':f'Unclassified {lt}','kind':'region','parent':'UNCLASS','asp':0}); seen.add(sid)
 ...
 for r in details:
  ...
  if sid not in seen:
   ...
   scopes.append({'id':sid,'label':text(r.get('Location')),'kind':classify(r.get('Location Type')),'parent':parent,'asp':0,'sourceType':text(r.get('Location Type'))}); seen.add(sid)
  key=(sid,ui)
